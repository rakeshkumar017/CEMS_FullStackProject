...
- **PgSQL** connection via pg client (configured in `config/db.js`).
...
│   └── main.ts
│
├── event-backend/           # Node.js backend
│   ├── config/db.js         # Database connection
│   ├── controllers/         # Auth, Event, Registration controllers
...
├── proxy.conf.json          # API proxy for Angular dev server
